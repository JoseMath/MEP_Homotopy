...
for i = 1:length(n_list)
    ...
    for n_simulation = 1:10
        ...
        tic;
        opts.singular = 1;
        [lambda,mu,X1,X2,~,~] = threepareig(A{1,1},A{1,2},A{1,3},A{2,1},A{2,2},A{2,3},opts);
        t{i,n_simulation} = toc;
        
        EigenValue{i,n_simulation} = [lambda,mu]; EigenValue{i,n_simulation} = mat2cell(EigenValue{i,n_simulation}, ones(size(lambda,1),1), k);        
        EigenVector = [X1.',X2.']; EigenVector = mat2cell(EigenVector, ones(size(lambda,1),1), ones(k,1)*3*n); EigenVector=cellfun(@transpose,EigenVector,'UniformOutput',false);
...
for i = 1:length(n_list)
    ...
    for n_simulation = 1:10
        A = random_matrices{i, n_simulation};
        tic;
        [EigenValue{i,n_simulation},EigenVector,SEigenValue,SEigenVector,TimeEachPath{i,n_simulation},NumNewtonEachPath{i,n_simulation},LastNewton,~,~] = FiberHomotopy(A, struct('tracker', 'conservative'));
        t{i,n_simulation} = toc;
